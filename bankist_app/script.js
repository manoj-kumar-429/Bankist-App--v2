...
navIcon.addEventListener(`click`, function () {
  ...
  // navLinks.classList.add(`navItemDisplay`);
  // nav.style.height = `60rem`;

  // navLinks.style.display = `none`;
  // navLinks.classList.remove(`navItemDisplay`);
  // nav.style.height = `9rem`;
});
// const hamburgerMenu = function (opacity) {
//   opacity === 0
//     ? (navIcon.style.display = `none`)
//     : (navIcon.style.display = `flex`);
// };
// let x = window.matchMedia('(max-width: 955px)');
// let count = 0;
...
// let y = window.matchMedia('(min-width: 955px)');
// // count = 0;
...
// navItem.forEach(navItem => (navItem.style.display = `none`));
/*

count++;if (count % 2 !== 0) {
        navItem.forEach(navItem => (navItem.style.display = `block`));
        navLinks.classList.add(`navItemDisplay`);
      } else navItem.forEach(navItem => (navItem.style.display = `none`));
    });
*/
